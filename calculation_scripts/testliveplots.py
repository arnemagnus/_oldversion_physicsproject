...
data = pg.gaussianFilter(data, (10, 10, 10))[int(frames/2):frames + int(frames/2)]
data[:, 15:16, 15:17] += 1
...
# pw = pg.plot()
# pw.disableAutoRange()
# pw.setRange(xRange = (x[0], x[-1]), yRange = (-1, 1))
# for i in range(int(np.ceil(tmax-tmin)/dt)):
#    pw.plot(x, np.sin(np.pi*x)*np.cos(np.pi*i*dt), clear=True)
